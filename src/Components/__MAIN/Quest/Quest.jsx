...
export default class Quest extends PureComponent {
  ...
  getQuest = async () => {
    ...
    if (this.questIndex > 4) {
      ...
      this.setState({
        quest: quest,
        answersList: quest.answers,
        duration: quest.duration,
      });
    }
  };

  timer = async (duration) => {
    let time = duration;
    setInterval(() => {
  ...
  componentDidMount = async () => {
    let id = this.props.match.params.examId;
    let questIndex = this.props.match.params.questIndex;
    questIndex > 4 ? this.setState({ quest: null }) : await this.getQuest();
    await this.timer(this.state.duration);
  };
  componentDidUpdate = async (prevProps, prevState) => {
    if (
      prevState.duration !== this.state.duration &&
      prevState.quest !== this.state.quest
    ) {
      if (this.state.duration === 0) {
        let randomNumber = Math.floor(Math.random() * 4);
        this.answer(randomNumber);
        await this.props.history.push(
          `/exam/${this.props.match.params.examId}/quest/${
            parseInt(this.props.match.params.questIndex) + 1
          }`
        );
      }
    }
    if (this.props.match.params.questIndex > 4) {
      ...
      await this.props.history.push(
        `/exam/${this.props.match.params.examId}/finished`
      );
    }
  };
    ...
    return (
      <div className="quest">
        ...
        {this.state.quest !== null ? (
            ...
            {this.state.answersList ? (
              this.state.answersList.map((answer, index) => {
                return (
                  <Link
                    key={index}
                    to={`/exam/${examId}/quest/${parseInt(questIndex) + 1}`}
          ...
          <Link to={`/exam/${examId}/finished`} className="results-page">
            Go to Result
          </Link>
        )}
      </div>
